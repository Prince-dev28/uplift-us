...
export default function MoodTracker() {
  ...
  const logMood = () => {
    ...
    addMood({ id: crypto.randomUUID(), date: new Date().toISOString(), mood: selected, note: note || undefined });
    setMoods(getMoods());
    setSelected(null);
    setNote("");
          ...
          {moodOptions.map(m => (
            <motion.button
              ...
              className={`flex flex-col items-center p-3 rounded-xl transition-all ${
                selected === m.value ? "bg-primary/10 ring-2 ring-primary shadow-soft scale-110" : "hover:bg-muted"
              }`}
            >
