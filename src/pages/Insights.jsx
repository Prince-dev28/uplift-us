...
const moodEmojis = ["\u{1F614}", "\u{1F61F}", "\u{1F610}", "\u{1F642}", "\u{1F60A}"];
export default function Insights() {
  ...
  >
          ...
          {moodTrend.length > 1 ? <ResponsiveContainer width="100%" height={200}>
  ...
  />
                <Tooltip formatter={(v) => [moodEmojis[v - 1] + ` (${v}/5)`, "Mood"]} />
                <Area type="monotone" dataKey="mood" stroke="hsl(234 62% 58%)" fill="url(#moodGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer> : <p className="text-sm text-muted-foreground h-[200px] flex items-center justify-center">Log more moods to see trends.</p>}
        </motion.div>
  ...
  >
          ...
          <div className="space-y-3">
            {moodDistribution.map((d, i) => <div key={i} className="flex items-center gap-3">
                <span className="text-xl w-8">{d.mood}</span>
                <div className="flex-1 bg-muted rounded-full h-6 overflow-hidden">
                  <motion.div
    initial={{ width: 0 }}
    animate={{ width: moods.length > 0 ? `${d.count / moods.length * 100}%` : "0%" }}
  ...
  >
        ...
        {triggers.length > 0 ? <div className="flex flex-wrap gap-2">
            {triggers.map((t) => <span key={t.trigger} className="px-4 py-2 rounded-full bg-calm-rose text-foreground text-sm font-medium flex items-center gap-2">
                {t.trigger} <span className="text-xs bg-destructive/10 text-destructive px-2 py-0.5 rounded-full">{t.count}×</span>
              </span>)}
          </div> : <p className="text-sm text-muted-foreground">No triggers detected yet. Add notes to your low mood entries to help identify patterns.</p>}
      </motion.div>
  ...
  >
        ...
        <div className="space-y-3">
          {insights.map((tip, i) => <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/50">
