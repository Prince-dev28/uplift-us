...
export default function Profile() {
  ...
  const [profilePic, setProfilePicState] = useState(getProfilePic());
  ...
  const handlePhoto = (e) => {
    ...
    reader.onload = () => {
      ...
      toast({ title: "Photo updated! \u{1F4F8}" });
    };
  ...
  return <div className="space-y-6 max-w-2xl mx-auto">
    ...
    /* Profile Picture */
        ...
        <div className="relative w-20 h-20 mx-auto mb-6 group cursor-pointer" onClick={() => fileRef.current?.click()}>
          {profilePic ? <img src={profilePic} alt="Profile" className="w-20 h-20 rounded-full object-cover border-2 border-border" /> : <div className="w-20 h-20 rounded-full gradient-calm flex items-center justify-center">
  ...
  />
    ...
    /* Assessment History */
      ...
      {assessments.length > 0 && <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
          <h2 className="font-display text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
            <History className="w-4 h-4 text-primary" /> Assessment History
          </h2>
          <div className="space-y-3">
            {assessments.map((a) => <div key={a.id} className="flex items-center justify-between p-3 rounded-xl bg-muted/50">
                <div>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${a.overallLevel === "Low" ? "bg-calm-green" : a.overallLevel === "Moderate" ? "bg-calm-peach" : "bg-calm-rose"} text-foreground`}>{a.overallLevel}</span>
