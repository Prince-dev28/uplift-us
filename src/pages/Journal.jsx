...
export default function Journal() {
  ...
  const save = () => {
    ...
    addJournal({ id: crypto.randomUUID(), date: (/* @__PURE__ */ new Date()).toISOString(), title: title.trim(), content: content.trim() });
  ...
  return <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        {!composing && <button onClick={() => setComposing(true)} className="flex items-center gap-1 px-4 py-2 rounded-xl gradient-calm text-primary-foreground text-sm font-semibold shadow-soft hover:opacity-90 transition-opacity">
            <Plus className="w-4 h-4" /> New Entry
          </button>}
      </div>

      <AnimatePresence>
        {composing && <motion.div
  ...
  />
      ...
      {entries.length === 0 && !composing ? <div className="bg-card rounded-2xl p-12 shadow-card border border-border text-center">
          <BookOpen className="w-10 h-10 text-muted-foreground mx-auto mb-3" />
          <p className="text-muted-foreground">No journal entries yet. Start writing!</p>
        </div> : <div className="space-y-4">
          {entries.map((e, i) => <motion.div
    key={e.id}
