  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 flex-wrap">
        {durations.map((d, i) => (
          <button key={d.label} onClick={() => selectDuration(i)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              i === selectedDuration ? "gradient-ocean text-secondary-foreground shadow-soft" : "bg-muted text-muted-foreground hover:bg-muted/80"
            }`}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        className={`rounded-2xl p-8 shadow-card border border-border flex flex-col items-center transition-colors duration-1000 ${
          isActive ? "bg-foreground/[0.02]" : "bg-card"
        }`}>
        {/* Animated Circle */}
        <div className="relative w-64 h-64 flex items-center justify-center">
          ...
          <svg className="absolute w-full h-full -rotate-90">
            ...
            <motion.circle cx="128" cy="128" r="110" fill="none" stroke="hsl(168 42% 52%)" strokeWidth="4"
              strokeDasharray={691} strokeDashoffset={691 * (1 - progress)} strokeLinecap="round"
        ...
        <div className="flex items-center gap-4 mt-6">
          <button onClick={() => { if (isComplete) reset(); else setIsActive(!isActive); }}
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
        ...
        <div className="grid grid-cols-4 gap-3">
          {ambiences.map((a, i) => (
            <button key={a.name} onClick={() => setSelectedAmbience(i)}
              className={`p-4 rounded-xl text-center transition-all ${
                i === selectedAmbience ? "bg-calm-green border-2 border-secondary shadow-soft" : "bg-muted hover:bg-muted/80 border-2 border-transparent"
              }`}>
