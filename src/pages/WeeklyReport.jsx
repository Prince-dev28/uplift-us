...
export default function WeeklyReport() {
  ...
  const avgMood = weeklyMoods.length > 0 ? (weeklyMoods.reduce((s, m) => s + m.mood, 0) / weeklyMoods.length).toFixed(1) : "N/A";
  ...
  const handleDownloadPDF = () => {
    const lines = [
      ...
      `Period: ${weekStart.toLocaleDateString()} - ${(/* @__PURE__ */ new Date()).toLocaleDateString()}`,
      "",
    ...
    a.href = url;
    a.download = `mindcare-report-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.txt`;
    a.click();
  ...
  >
      ...
      <p className="text-sm text-muted-foreground">
        ...
        {weekStart.toLocaleDateString("en", { month: "long", day: "numeric" })} — {(/* @__PURE__ */ new Date()).toLocaleDateString("en", { month: "long", day: "numeric", year: "numeric" })}
      </p>
  ...
  />
            <Tooltip formatter={(v) => [v > 0 ? `${moodEmojis[Math.round(v) - 1]} (${v}/5)` : "No data", "Mood"]} />
  ...
  >
        ...
        {weeklyJournals.length > 0 ? <div className="space-y-3">
            {weeklyJournals.map((j) => <div key={j.id} className="p-3 rounded-xl bg-muted/50">
          ...
          </div> : <p className="text-sm text-muted-foreground">No journal entries this week.</p>}
      </motion.div>
    </div>;
}
