...
function getSuggestions(level, stress, anxiety, depression) {
  const s = [];
  if (level === "Low") {
...
export default function Assessment() {
  ...
  const submit = () => {
    ...
    addAssessment(r);
    setResult(r);
    toast({ title: "Assessment complete! \u{1F4CA}" });
  };
  if (result) {
    return <div className="space-y-6 max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <div className={`rounded-2xl p-6 text-center mb-6 ${result.overallLevel === "Low" ? "bg-calm-green" : result.overallLevel === "Moderate" ? "bg-calm-peach" : "bg-calm-rose"}`}>
      ...
      { label: "Depression", score: result.depressionScore, max: 12, level: getLevel(result.depressionScore) }
    ].map((s) => <div key={s.label} className="bg-card rounded-2xl p-4 shadow-card border border-border text-center">
                <div className="text-2xl font-bold text-foreground">{s.score}</div>
                <div className="text-xs text-muted-foreground">{s.label}</div>
                <div className={`text-xs font-semibold mt-1 ${s.level === "Low" ? "text-secondary" : s.level === "Moderate" ? "text-accent" : "text-destructive"}`}>{s.level}</div>
              </div>)}
          </div>
          ...
          <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
            ...
            <ul className="space-y-2">
              {result.suggestions.map((s, i) => <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
        ...
        <motion.div key={step} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.2 }}>
          <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
            ...
            <div className="space-y-2">
              {q.options.map((opt, i) => <button
    key={i}
    onClick={() => answer(i)}
    className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition-all border ${answers[step] === i ? "gradient-calm text-primary-foreground border-transparent shadow-soft" : "bg-muted text-foreground border-border hover:border-primary/30"}`}
  >
                  {opt}
                </button>)}
            </div>
      ...
      <div className="flex gap-3">
        {step > 0 && <button onClick={() => setStep(step - 1)} className="flex items-center gap-1 px-5 py-3 rounded-xl border border-border text-sm font-semibold text-foreground hover:bg-muted transition-colors">
