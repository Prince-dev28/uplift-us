...
export default function Breathing() {
  ...
  const getPhaseTime = useCallback((p: Phase) => {
    switch (p) {
      ...
      case "rest": return technique.rest;
    }
  }, [technique]);

  const getNextPhase = useCallback((p: Phase): Phase => {
    switch (p) {
      case "inhale": return technique.hold > 0 ? "hold" : "exhale";
      case "hold": return "exhale";
      case "exhale": return technique.rest > 0 ? "rest" : "inhale";
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 flex-wrap">
        {techniques.map((t, i) => (
          <button key={t.name} onClick={() => { setSelectedTechnique(i); reset(); }}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              i === selectedTechnique ? "gradient-calm text-primary-foreground shadow-soft" : "bg-muted text-muted-foreground hover:bg-muted/80"
            }`}>
            {t.name}
          </button>
        ))}
      ...
      {/* Breathing Animation */}
      <motion.div className="bg-card rounded-2xl p-8 shadow-card border border-border flex flex-col items-center"
